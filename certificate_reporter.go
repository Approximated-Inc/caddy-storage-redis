...
func (a *CertificateReporter) Provision(ctx caddy.Context) error {
	if err := a.provisionConfiguration(ctx); err != nil { return err }
	a.logger = ctx.Logger()
	events, err := ctx.App("events")
	if err != nil { return errors.New("certificate health lifecycle events unavailable") }
	app, ok := events.(*caddyevents.App)
	if !ok { return errors.New("certificate health lifecycle events unavailable") }
...
func (a *CertificateReporter) Start() error {
	a.mu.Lock()
	defer a.mu.Unlock()
	if a.released || a.state.Load() != nil { return nil }
...
func (a *CertificateReporter) Handle(_ context.Context, event caddy.Event) error {
	if event.Origin() != nil { return nil }
...
func (a *CertificateReporter) Stop() error {
	a.mu.Lock()
	if a.released { a.mu.Unlock(); return nil }
	a.released = true
	s := a.state.Load()
	a.mu.Unlock()
	if s == nil { return nil }
...
func (a *CertificateReporter) Stats() CertificateReporterStats {
	if s := a.state.Load(); s != nil { return s.snapshot() }
...
func (s *certificateReporterState) start() {
	...
	if s.started || s.ctx.Err() != nil { return }
	s.started = true
	s.wg.Add(2)
	for i := 0; i < 2; i++ { go s.run() }
}
func (s *certificateReporterState) TryEnqueue(c MismatchCandidate) bool {
	if !s.mu.TryLock() { s.dropped.Add(1); return false }
	defer s.mu.Unlock()
	...
	if len(s.dedupe) >= certificateDedupeCapacity {
		...
		for k, expiry := range s.dedupe { if earliest.IsZero() || expiry.Before(earliest) { oldest, earliest = k, expiry } }
...
func (s *certificateReporterState) prune(now time.Time) {
	for key, expiry := range s.dedupe { if !now.Before(expiry) { delete(s.dedupe, key) } }
	for host, work := range s.work { if !work.busy && !now.Before(work.expires) { delete(s.work, host) } }
}
func (s *certificateReporterState) run() {
	...
	for {
		...
		for _, w := range s.work {
			if !w.busy && !now.Before(w.due) && (selected == nil || w.due.Before(selected.due)) { selected = w }
		}
		if selected != nil { selected.busy = true }
		...
		if selected != nil {
			...
			s.mu.Lock()
			selected.busy = false
			if !keep || s.ctx.Err() != nil || !time.Now().Before(selected.expires) { delete(s.work, selected.candidate.Hostname) }
			s.mu.Unlock()
			continue
		}
		select { case <-s.ctx.Done(): return; case <-s.wake: case <-ticker.C: }
	}
}
func (s *certificateReporterState) process(w *certificateWork) bool {
	...
	if w.event == nil {
		...
		if ctx.Err() != nil { return false }
		if result.Result == "" {
			if w.acceptance == nil { return false }
			w.due = time.Now().Add(30*time.Second); return true
		}
		if result.Result == TLSValid && w.acceptance == nil { return false }
		observed := result.CheckedAt.UTC().Truncate(time.Second)
		if !observed.After(w.lastObservation) { w.due = time.Now().Add(time.Second); return true }
		id, err := certificateUUID()
		if err != nil { s.failures.Add(1); return false }
		...
		if w.acceptance == nil { e.ObservedAt = w.birth.UTC().Truncate(time.Second).Format(time.RFC3339) }
		...
		if w.acceptance != nil { e.ReportID, e.VerificationNonce = w.acceptance.ReportID, w.acceptance.VerificationNonce }
...
func (s *certificateReporterState) deliver(ctx context.Context, event *certificateEvent) (*certificateAcceptance, bool) {
	...
	if err != nil || len(body) > 8192 { s.failures.Add(1); return nil, false }
	request, err := http.NewRequestWithContext(ctx, http.MethodPost, s.endpoint, bytes.NewReader(body))
	if err != nil { s.failures.Add(1); return nil, false }
	request.Header.Set("Content-Type", "application/json")
	request.Header.Set("apx-proxy-server-id", strconv.FormatInt(s.cluster, 10))
	request.Header.Set("apx-key", s.token)
	response, err := s.client.Do(request)
	if err != nil { s.failures.Add(1); return nil, true }
	defer response.Body.Close()
	if response.StatusCode == http.StatusNoContent { s.delivered.Add(1); return nil, false }
	...
	if err != nil { s.failures.Add(1); return nil, true }
	if len(payload) > 8192 { s.failures.Add(1); return nil, false }
...
func parseCertificateAcceptance(payload []byte, now time.Time) (*certificateAcceptance, error) {
	...
	first, err := d.Token()
	if err != nil || first != json.Delim('{') { return nil, errors.New("invalid acceptance") }
	seen := make(map[string]bool, 4)
	a := new(certificateAcceptance)
	for d.More() {
		token, err := d.Token()
		key, ok := token.(string)
		if err != nil || !ok || seen[key] { return nil, errors.New("invalid acceptance") }
		seen[key] = true
		switch key {
		case "report_id": err = d.Decode(&a.ReportID)
		case "verification_nonce": err = d.Decode(&a.VerificationNonce)
		case "check_after_seconds": err = d.Decode(&a.CheckAfterSeconds)
		case "expires_at": err = d.Decode(&a.ExpiresAt)
		default: return nil, errors.New("invalid acceptance")
		}
		if err != nil { return nil, errors.New("invalid acceptance") }
	}
	if _, err := d.Token(); err != nil { return nil, errors.New("invalid acceptance") }
	if _, err := d.Token(); err != io.EOF { return nil, errors.New("invalid acceptance") }
...
func (s *certificateReporterState) stop() {
	...
	s.mu.Lock()
	clear(s.work); clear(s.dedupe)
	s.mu.Unlock()
}
func lowerHex(s string, size int) bool {
	if len(s) != size { return false }
	for _, ch := range s { if !(ch >= '0' && ch <= '9' || ch >= 'a' && ch <= 'f') { return false } }
	return true
}
